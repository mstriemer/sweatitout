...
from models import Course, DBCourse, CourseDay, Location
from database import db_session

def parse_time(time_string):
    hour, minute = time_string.split(':')
    minute = minute[:2] # drop the am/pm
    return int(hour) + 1200 + int(minute)

def parse_human_date(date_string, year):
    suffices = ['st', 'nd', 'rd', 'th']
    parsed = None
    for suffix in suffices:
        date_format = '%B %d{suffix}'
        date_format += ', %Y' if year else ''
        try:
            parsed = datetime.strptime(date_string,
                                       date_format.format(suffix=suffix))
            if not year:
                parsed = parsed.replace(year=2013)
            break
        except ValueError:
            pass
    if parsed is None:
...
def make_day(day):
    return CourseDay(name=day.name,
...
for course in all_courses:
    location = (db_session.query(Location)
                          .filter(Location.name == course.location)
                          .first())
    if location is None:
        location = Location(name=course.location,
                            map_url=course.map_url,
    ...
    db_course = DBCourse(
        ...
        start_date=parse_human_date(course.start_date, year=False),
        end_date=parse_human_date(course.end_date, year=True),
        cost=course.cost,
        has_space=course.has_space,
        allow_assessments=course.has_space,
        location = location,
    )
    db_course.days = [make_day(day) for day in course.days]
